...
class RunStore:
    ...
    def seed_topics(self, topics: List[Dict[str, Any]]) -> None:
        """Seed canonical topics, updating existing entries if definition changed."""
        conn = self._get_conn()
        for t in topics:
            slug = t.get("slug")
            title = t.get("title")
            ...
            conn.execute("""
                INSERT INTO topics (slug, title, definition_md, equations_md, edition, created_at, updated_at, tags_json)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(slug) DO UPDATE SET
                    title = excluded.title,
                    definition_md = excluded.definition_md,
                    equations_md = excluded.equations_md,
