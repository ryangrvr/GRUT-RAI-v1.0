...
def test_publish_and_open_snapshot():
    # Dev-only: require explicit opt-in to avoid CI/environment breakage.
    # (Playwright may be installed without browsers in minimal environments.)
    if os.environ.get("RUN_E2E") not in ("1", "true", "TRUE", "yes", "YES"):
        pytest.skip("E2E test is opt-in; set RUN_E2E=1 to enable")
    ...
    try:
        with sync_playwright() as p:
            try:
                ...
                msg = str(e)
                # Common in CI/dev containers: playwright installed but browsers not downloaded.
                if "Executable doesn't exist" in msg or "playwright install" in msg:
                    pytest.skip("Playwright browsers not installed; run `playwright install` to enable E2E")
                raise
