...
def test_seed_topics_created():
    ...
    # Ensure seeded topics exist (core + evidence + architecture)
    assert 'tau0-memory-window' in slugs
    assert 'nis-certificates' in slugs
    assert 'alpha-mem' in slugs
    assert 'memory-kernel' in slugs
    assert 'sovereign-firewall' in slugs
    assert len(topics) >= 20  # expanded from 12 to 25+ topics
