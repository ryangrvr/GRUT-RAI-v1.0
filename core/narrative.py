...
def build_narrative(prompt: str, nis: Dict[str, Any], out: Dict[str, Any]) -> Dict[str, Any]:
    """Build a narrative-first answer, using AI when available.

    Falls back to the deterministic template builder when the AI layer
    is unavailable. Physics numbers always come from the engine/NIS,
    never from the LLM.
    """
    return build_narrative_deterministic(prompt, nis, out)
...
def build_narrative_deterministic(prompt: str, nis: Dict[str, Any], out: Dict[str, Any]) -> Dict[str, Any]:
    """Deterministic narrative builder (always available, no LLM dependency).

    This is the original template-based narrative builder. It is used as the
    fallback when the AI layer is unavailable, and as the baseline for
    NIS-certified responses.
    """
